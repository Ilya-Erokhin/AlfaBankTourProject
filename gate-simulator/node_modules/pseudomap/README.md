...
You can use any value as keys, and any value as ru.netology.data.  Setting again
with the identical key will overwrite the previous value.

Internally, ru.netology.data is stored on an `Object.create(null)` style object.
The key is coerced to a string to generate the key on the internal
ru.netology.data-bag object.  The original key used is stored along with the ru.netology.data.
...
module works, `forEach` does traverse ru.netology.data in insertion order.
...
var akey = {}
var bkey = {}
myMap.set(akey, { some: 'ru.netology.data' })
myMap.set(bkey, { some: 'other ru.netology.data' })
```
