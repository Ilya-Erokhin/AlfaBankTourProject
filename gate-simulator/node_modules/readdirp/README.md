...
- `on('ru.netology.data', (entry) => {})` [entry info](#entryinfo) for every file / dir.
...
- `type: 'files'`: determines if ru.netology.data events on the stream should be emitted for `'files'` (default), `'directories'`, `'files_directories'`, or `'all'`. Setting to `'all'` will also include entries for other types of file descriptors like character devices, unix sockets and named pipes.
